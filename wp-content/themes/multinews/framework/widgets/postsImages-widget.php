...
class mom_widget_posts_images extends WP_Widget {
	function mom_widget_posts_images() {
		...
		$this->WP_Widget('momizat-Posts-Images',__('Momizat - Posts in images','framework'),$widget_ops);
		}
		...
		<?php if( mom_post_image() != false ) { ?>
			<li class="simptip-position-top simptip-movable half-arrow simptip-multiline" data-tooltip="<?php the_title(); ?>" itemscope="" itemtype="http://schema.org/Article">
				<figure class="post-thumbnail"><a itemprop="url" href="<?php the_permalink(); ?>" rel="bookmark">
                    <img itemprop="image" class="tip_n" src="<?php echo mom_post_image('postpicwid-thumb'); ?>" data-hidpi="<?php echo mom_post_image('media1-thumb'); ?>" alt="<?php the_title(); ?>" title="<?php the_title(); ?>">
                </a></figure>
			</li>
			<?php } ?>
			<?php endwhile; ?>
